        ...
        for y in range(y_min, y_max):
            for x in range(x_min, x_max):  # Ensure x-1 is valid
                if x < x_min or x >= x_max:
                    continue
                if deltaI[t, y, x-2] < 0 and deltaI[t, y, x-1] < 0 and deltaI[t, y, x] >= 0 and deltaI[t, y, x+1] >= 0:
...
def main():
    ...
    horiz_y_min, horiz_y_max, horiz_x_min, horiz_x_max = 100, 800, 1440, 2500    # region for horizontal plane shadow line detection
    vert_y_min, vert_y_max, vert_x_min, vert_x_max = 1640, 2000,1600, 2500    # region for vertical plane shadow line detection
    ...
    calib_extrinsic = np.load('../data/scene6/extrinsic_calib.npz')
    ...
    for t in range(T-1):
        ...
        P12 = compute_shadow_line_3D(line_h, 1600, 1900, mtx, dist, rmat_h, tvec_h)
        # P3,P4 on vertical plane line
        P34 = compute_shadow_line_3D(line_v, 200, 600, mtx, dist, rmat_v, tvec_v)
    ...
    # Example crop:
    x1,x2,y1,y2 = 1520, 2410, 890, 1600
