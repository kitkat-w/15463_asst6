...
def estimate_shadow_edges(deltaI, y_min, y_max, x_min, x_max):
    ...
    for t in range(1, T):  # Start from t=1 to ensure valid indexing
        ...
        for y in range(y_min, y_max):
            for x in range(1, W):  # Ensure x-1 is valid
                if x < x_min or x >= x_max:
                    continue
                if deltaI[t, y, x-1] < 0 and deltaI[t, y, x] >= 0:
...
def main():
    ...
    # plt.imshow(deltaI[40], cmap='gray')
    # plt.show()
    ...
    horiz_y_min, horiz_y_max, horiz_x_min, horiz_x_max = 0, 300, 250, 800    # region for horizontal plane shadow line detection
    vert_y_min, vert_y_max, vert_x_min, vert_x_max = 675, 765, 200, 820    # region for vertical plane shadow line detection
    ...
    calib_intrinsic = np.load('frog/intrinsic_calib.npz')
    mtx = calib_intrinsic['mtx']
    dist = calib_intrinsic['dist']
    calib_extrinsic = np.load('frog/extrinsic_calib.npz')
    ...
    for t in range(T-1):
        ...
        P12 = compute_shadow_line_3D(line_h, 650, 763, mtx, dist, rmat_h, tvec_h)
        # P3,P4 on vertical plane line
        P34 = compute_shadow_line_3D(line_v, 10, 250, mtx, dist, rmat_v, tvec_v)
    ...
    np.save('frog/all_pts.npz', p1 = all_p1, p2 = all_p2, p3 = all_p3, p4 = all_p4)
    np.save('frog/all_planes.npz', planes = all_planes)
    ...
    for yy in range(y1,y2):
        for xx in range(x1,x2):
            ...
            if abs(diff) < thresh:
                continue
            if tt<46 or tt>=T-1 or all_planes[int(tt)] is None:
                # no valid shadow crossing
                continue
            P1, normal = all_planes[int(tt)]
            ...
            intensity = color_img[yy,xx]
            Cpts.append(intensity)

    Zpts = np.array(Zpts)
    Cpts = np.array(Cpts)

    np.savez('frog/frog_pts.npz', Zpts=Zpts, Cpts=Cpts)
